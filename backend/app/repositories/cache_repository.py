...
class SampleRepository:
    ...
    def _build_dummy_sector(self, sector_id: str) -> dict[str, Any]:
        ...
        if stocks is None:
            ...
            stocks = list(stocks)
        stocks.extend(
            self._fallback_stock_tile(tile, len(stocks) + index)
            for index in range(max(0, 20 - len(stocks)))
        )
        stocks = [
            {
                **stock,
    ...
    def _fallback_stock_tile(self, tile: dict[str, Any], index: int) -> dict[str, Any]:
        current_price = 18000 + index * 7400
        change_rate = round(float(tile["average_change_rate"]) + (index - 9) * 0.28, 2)
        ...
        return {
            "stock_code": f"{(sum(ord(char) for char in tile['sector_id']) + index) % 1000000:06d}",
            "stock_name": f"{tile['sector_name']} Sample {index + 1}",
            ...
            "average_volume": average_volume,
            "volume_growth_rate": round((volume - average_volume) / average_volume * 100, 2),
            "trading_value": max(1, int(tile["total_trading_value"]) // (index + 3)),
            "market_cap": max(1, int(tile["total_market_cap"]) // (index + 2)),
    ...
    def _build_dummy_stock(self, stock_code: str) -> dict[str, Any]:
        ...
        return {
            "selection": {
                "level": "stock",
                "market": "KR",
                "sector_id": matched_sector_id or "information-technology",
                "stock_code": stock_code
            },
            "as_of": "2026-05-10 15:30:00",
            "identity": {
