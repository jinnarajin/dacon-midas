...
- [x] 시장맵 크기 기준을 시가총액/거래대금으로 전환할 수 있게 만든다.
- [x] 시장맵 색상 기준을 등락률/변동성으로 전환할 수 있게 만든다.
- [x] 섹터 클릭 시 섹터 내부 종목맵으로 전환한다.
- [x] 종목 클릭 시 Overview로 진입한다.
- [x] 차트 기간 선택 `1D / 1W / 1M / 3M / 1Y`를 만든다.
- [x] 데이터 결측, 뉴스 없음, 차트 없음 상태를 UI에서 처리한다.
